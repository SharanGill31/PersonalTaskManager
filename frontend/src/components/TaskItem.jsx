...
<div className="flex-1 min-w-0">
  ...
  {task.decription && <p className={TI_CLASSES.description}>
    {task.decription}</p>}
</div>
    </div>
              ...
              <div className={`${TI_CLASSES.menuDropdown}`}>
                {MENU_OPTIONS.map((opt) => (
                  <button
                    ...
                    className="w-full px-3 py-2 sm:px-4 sm:py-2 text-left text-xs sm:text-sm hover:bg-purple-50 flex items-center justify-center gap-2 transition-colors duration-200"
                  >
                    {opt.icon}
                    {opt.label}
                  </button>
