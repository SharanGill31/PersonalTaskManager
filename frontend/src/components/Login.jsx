...
import { BUTTON_CLASSES, INPUT_WRAPPER } from '../assets/dummy'
...
const Login = ({ onSubmit, onSwitchMode }) => {
  ...
  useEffect(() => {
    ...
    if (token && userId) { // Added userId check
      (async () => {
        try {
  ...
  try {
    console.log('Login request payload:', formData); // Add debug log for payload
    const { data } = await axios.post(`${url}/api/user/login`, formData, {
      headers: { 'Content-Type': 'application/json' }, // Explicitly set Content-Type
    });
    console.log('Login response:', data); // Add debug log for response
    ...
    console.error('Login error:', error.response?.data, error.message); // Improved error logging
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
        {fields.map(({ name, type, placeholder, icon: Icon, isPassword }) => (
          <div key={name} className={INPUT_WRAPPER}> 
            ...
            {isPassword && (
              <button
                type="button"
                onClick={() => setShowPassword(prev => !prev)} // Removed extra parentheses
        ...
        <div className="flex items-center">
          <input
            ...
            id="rememberMe"
            checked={rememberMe}
            onChange={() => setRememberMe(prev => !prev)} // Consistent state update pattern
