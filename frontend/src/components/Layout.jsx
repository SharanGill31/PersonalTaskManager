...
import Sidebar from './Sidebar' // Keep the import
    ...
    const fetchTasks=useCallback(async()=>{
        ...
        try {
          ...
          if(!token) throw new Error("No auth token found")
            ...
            const {data}=await axios.get("https://personaltaskmanager-backend-c1vs.onrender.com/api/tasks/gp", {
...
const completionPercentage = Math.round((completedTasks / totalCount) * 100) || 0;

return {
  totalCount,
  pendingCount,
  completionPercentage,
  completedTasks // Make sure to include this for the StatCard
};
}, [tasks])
...
if(error) return(
    <div className='min-h-screen bg-gray-50 p-6 flex items-center justify-center'>
        <div className='bg-red-50 test-red-600 p-4 rounded-xl border border-red-100 max-w-md'>
            <p className='font-medium mb-2'>Error loading tasks</p>
            <p className='text-sm'>{error}</p>
            <button onClick={fetchTasks} className='mt-4 py-2px-4  bg-red-100 text-red-700 rounded-lg text-sm font-medium hover:bg-red-200 transition-colors'>
                Try Again
            </button>
        </div>
    </div>
  ...
  return (
    <div className='min-h-screen bg-gray-50'>
      ...
      <Sidebar user={user} tasks={tasks} /> {/* Pass tasks instead of task */}
...
<div className="space-y-2 sm:space-y-3">
  <div className="flex items-center justify-between text-gray-700">
    ...
    <span className='text-xs bg-purple-100 text-purple-700 px-1.5 py-o.5 sm:px-2 rounded-full'>
    ...
    <div className='flex gap-1.5 items-center'>
        <div className='flex-1 h-2 sm:h-3 bg-purple-100 rounded-full overflow-hidden'>
            <div className='h-full bg-gradient-to-r from-fuchsia-500 to-purple-600 transition-all duration-500' style={{width:`${stats.completionPercentage}% `}}/>
                ...
                <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100'>
                    <h3 className='text-base sm:text-large font-semibold mb-3 sm:mb-4 text-gray-800 flex items-center gap-3 '>
